...
\draw[arrow] (a1) -- node[midway, above] {$a_2$} (adots);
\draw[arrow] (adots) -- node[midway, above] {$a_{n-1}$} (a3);
\draw[arrow] (a3) -- node[midway, above] {$a_n$}  (scr);
...
\caption{Cayley graph representation of the scramble and solution structure. The forward path $w$ maps $e \to X$, while $v$ maps $X \to e$. The diagram also shows the induced inverse traversals $w^{-1}$ and $v^{-1}$.}
\label{fig:paths3}
\end{center}
\end{figure}
